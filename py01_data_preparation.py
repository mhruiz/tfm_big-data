...
def main():
    ...
    if LOAD_NOISE_DATA:
        ...
        dataset = dataset[dataset['daypart'] != 'T']

        # 0: D (diurno), 1: N (nocturno), 2: E (vespertino)
        dataset['daypart'] = np.vectorize(lambda x: 0 if x == 'D' else 1 if x == 'N' else 2)(dataset['daypart'])
        ...
        dayparts, part_hours = list(zip(*daypart_hour))
